...
from train_env.myTrainEnv import TrainEnv
...
parser.add_argument('--reward_path', type=str, default='./output_images/avg_reward.svg')
parser.add_argument('--epsilon_path', type=str, default='./output_images/epsilon.svg')
parser.add_argument('--energy_path', type=str, default='./output_images/energy.svg')
...
def main():
    ...
    agent = DQN(alpha=0.0001, state_dim=4, action_dim=4,
                ...
                epsilon=0.25, eps_end=0.25, eps_dec=0, max_size=15000, batch_size=512)
    ...
    for episode in range(args.max_episodes):
        ...
        while True:
            ...
            observation_, reward, done, info = env.step(action)
            env.ptest.append(env.state.pos)
            env.vtest.append(env.state.v * 3.6)
            total_reward += reward
            observation = observation_
            if done:
                if total_reward > best_reward:
                    print("best_reward={}, episode{}".format(total_reward, episode+1))
                    mybasic.SL_Grad_curve(env.railway.slpos, env.railway.slval,
                                          env.railway.gradpos, env.railway.gradval)
                    plt.plot(env.ptest, env.vtest)
                ...
                break
        EnergyList.append(info["E"])
        total_rewards.append(total_reward)
        avg_reward = np.mean(total_rewards[-100:])
        avg_rewards.append(avg_reward)
        eps_history.append(agent.epsilon)
        print('EP:{} reward:{} avg_reward:{} epsilon:{} step:{} info:{}'.
              format(episode + 1, total_reward, avg_reward, agent.epsilon,
                     env.timestep, env.info))
        ...
        if (episode + 1) % 100 == 0:
            ...
            df.columns = ['positions', 'speed', 'time', 'action',
                          'Rstop', 'Rtime', 'Renergy', 'Radd', 'Rspeed']
        ...
        if (episode + 1) % 500 == 0:
            ...
            plot_learning_curve(episodes, EnergyList, 'Energy', 'energy(kw·h)', args.energy_path)
            plt.clf()
